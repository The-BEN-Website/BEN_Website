      ...
      <form id="form" className="w-full h-fit rounded flex flex-col gap-6 m-auto" onSubmit={handleSubmit} ref={form}>
        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-10">
          ...
          <span className="flex flex-col md:w-full lg:w-full">
            ...
            <input type="date" placeholder="Enter Your Date of Birth" id="date" className="text-gray-400 border border-gray-200 rounded w-full py-3 px-2 font-my_font" ref={dateOfBirthRef} value={message} onChange={event => setMessage(event.target.value)} name="Date of Birth"/>
            <ValidationError 
            prefix="dateOfBirth" 
            field="dateOfBirth"
            errors={state.errors}
        ...
        <button className='bg-my-red text-white px-6 w-full md:w-1/2 py-3 font-semibold rounded-sm font-my_font m-auto mb-5' type="submit" disabled={!message} onClick={() => state.submitting}>SUBMIT</button>
      </form>
    </section>
  )
}
